...
% Plot the Lorenz curves and calculate the Gini coefficients
Mu_trans = Mu';
...
title(['Gini Coefficient of Earnings = ', num2str(gini_earnings),...
       ' when there are ', num2str(num_a), ' grid points in a'])
val_wealth = [a' + y_s(1); a' + y_s(2)];
val_wealth(val_wealth < 0) = 0;
figure
[gini_wealth, l_wealth] = gini(pop, val_wealth, true);
title(['Gini Coefficient of Wealth = ', num2str(gini_wealth),...
       ' when there are ', num2str(num_a), ' grid points in a'])